...
function Pilot_One_fMRI_event_Multi_Cont(test, subj_id, run)
...
my_key = 'b'; % What key gives a response. Corresponds to the response buttons blue, yellow, green, red (1,2,3,4 from index to pinky using right hand)
my_key_2 = 'y';
my_key_3 = 'g';
my_key_4 = 'r';
...
file_to_save = ['Cont_1_event_Kete_Teke_Multi_subj_' num2str(subj_id) '_fmri_run_' num2str(run)  '_data.mat']; 
...
Sound_DIR = [STIM_DIR filesep 'Sounds_Multi'];
Image_DIR = [STIM_DIR filesep 'Images_Multi'];
Run_DIR = [pwd filesep 'FMRI_Cont_Multi_Runs.mat'];
Rest_DIR = [pwd filesep 'FMRI_Cont_Multi_Blanks_3_26_17.mat'];
% Choose run TR
% 
%%%Attend auditory runs are indexed below:
attend_aud_vis = [1,3,4,8];
%%%Attend visual runs are 2,5,6, and 7.
...
%Only 2 images here
    which_image = rot90(fullfact([2 2]),2) ;
...
for c  = 1:size(find(mixtr(:,1)~=0),1)
 ...
 t = mixtr(c,3)==which_image(:,1) & mixtr(c,4)==which_image(:,2);
...
which_wav = rot90(fullfact([2 2]),2) ; %Puts the 4 wavefiles into indices 1-4, since design matrix calls coordinate pairs
...
%         vbl =  Screen('Flip',wPtr,starttime-FlipInt_flex_time + rest_duration); %swaps backbuffer to frontbuffer; gets rid of "REST"
     ...
     Screen('DrawTexture', wPtr, ImageIndex(mixtr(sound_stim, 3),mixtr(sound_stim, 4)), [], ImRect, [], filtermode);
                else
               DrawFormattedText(wPtr, fixation, xtextpos, ytextpos, textcolor);
                end
...
%                 WaitSecs(0.001);
    ...
    subj_data.probe_onset(trial) = timedstart;
        ...
        if did_subj_respond == 0,
            did_subj_respond = getKeyResponse(my_key, my_key_2, my_key_3, my_key_4, r_count);
        else
            if Flipped_it==0
...
% % % % % % % %
    ...
    function out = getKeyResponse(my_key, my_key_2, my_key_3, my_key_4, r_count)
        KEY1=KbName(my_key);
